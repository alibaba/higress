...
func TestPublishA2ACreatesServiceRoutePluginAndAttachment(t *testing.T) {
	...
	if requests[2].body["phase"] != "AUTHN" || requests[2].body["priority"] != float64(300) {
		t.Fatalf("unexpected plugin: %#v", requests[2].body)
	}
	config := requests[3].body["configurations"].(map[string]interface{})
	agent := config["agent"].(map[string]interface{})
	if agent["id"] != "weather" {
		t.Fatalf("unexpected attachment: %#v", requests[3].body)
	}
}
...
func TestBuildA2APublicationRejectsInvalidURL(t *testing.T) {
	if _, _, _, _, _, err := buildA2APublication("weather", "file:///tmp/agent"); err == nil {
