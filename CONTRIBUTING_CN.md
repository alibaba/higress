...
任何使用 AI 或编程代理的 PR 都必须披露提示词/指令，并提供包含关键决策、主要更改及重要限制的 AI 辅助工作总结。新增独立 Wasm 或 golang-filter 插件还必须在插件的 `design/` 目录中保留设计文档。未满足门禁的实质性代理辅助 PR 将被安排为**较低审查优先级**，并且**不保证**维护者会及时审查。
