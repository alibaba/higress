...
func main() {
	...
	flag.Parse()
	tr := &http.Transport{ForceAttemptHTTP2: true, TLSClientConfig: &tls.Config{InsecureSkipVerify: true}, DialContext: func(ctx context.Context, network, address string) (net.Conn, error) {
		_, port, _ := net.SplitHostPort(address)
		return (&net.Dialer{}).DialContext(ctx, network, net.JoinHostPort(*node, port))
	}}
	client := &http.Client{Transport: tr, Timeout: 5 * time.Second}
	failures := 0
	check := func(name string, ok bool, detail interface{}) {
		json.NewEncoder(os.Stdout).Encode(obj{"case": name, "pass": ok, "detail": detail})
		if !ok {
			failures++
		}
	}
	call := func(host, path, method string, params obj, trailers bool) (*http.Response, []byte, error) {
		...
		if method != "" {
			verb = "POST"
			b, _ := json.Marshal(obj{"jsonrpc": "2.0", "id": "verify-1", "method": method, "params": params})
			body = bytes.NewReader(b)
		}
		req, _ := http.NewRequest(verb, "https://"+host+":30443"+path, body)
		req.Header.Set("Content-Type", "application/json")
		req.Header.Set("A2A-Version", "0.3")
		req.Header.Set("X-Higress-A2A-Method", "spoofed")
		if trailers {
			req.ContentLength = -1
			req.Trailer = http.Header{"X-Demo-Request-Trailer": []string{"complete"}}
		}
	...
	for _, source := range []string{"k8s", "remote"} {
		...
		req, _ := http.NewRequest("POST", "https://"+host+":30443/", bytes.NewReader(data))
		req.Header.Set("Content-Type", "application/json")
		start := time.Now()
		sr, se := client.Do(req)
		if se != nil {
			check(source+" stream", false, se.Error())
		} else {
			...
			sr.Body.Close()
			check(source+" stream", scanner.Err() == nil && count == 2 && first < 500*time.Millisecond && time.Since(start) >= 600*time.Millisecond, obj{"first_ms": first.Milliseconds(), "total_ms": time.Since(start).Milliseconds(), "events": events})
		}
		for i := 0; i < 3; i++ {
	...
	if failures > 0 {
		fmt.Fprintf(os.Stderr, "%d assertions failed\n", failures)
		os.Exit(1)
	}
}
