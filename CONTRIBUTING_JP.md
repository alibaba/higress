...
AI またはコーディングエージェントを使用したすべての PR は、プロンプト／指示を開示し、重要な判断、主な変更、重要な制約を含む AI 支援作業の要約を提出する必要があります。新規の独立した Wasm または golang-filter プラグインでは、プラグインの `design/` ディレクトリにも設計文書を残す必要があります。ゲートを満たさない、エージェントが実質的に関与した PR は**レビュー優先度が低く**なり、メンテナーによる迅速なレビューは**保証されません**。
