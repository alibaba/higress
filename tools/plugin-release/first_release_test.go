...
func TestFirstManagedReleasePlansExactlyFromV223DespiteMissingVersions(t *testing.T) {
	...
	if missingHistoricalVersions != 16 || len(previous.Plugins) != 43 || deferredAlpha != 2 {
		t.Fatalf("v2.2.3 fixture drift: eligible=%d missing VERSION=%d deferred=%d, want 43/16/2", len(previous.Plugins), missingHistoricalVersions, deferredAlpha)
	}
	...
	if len(first.Deferred) != 2 || first.Deferred[0].LogicalID != "ai-endpoint-picker" ||
		first.Deferred[1].LogicalID != "replay-protection" ||
		first.Deferred[0].Version != "1.0.0-alpha" || first.Deferred[1].Version != "1.0.0-alpha" ||
		first.Deferred[0].Reason != "alpha-prerelease" || first.Deferred[1].Reason != "alpha-prerelease" {
