...
func (m *qwenProviderInitializer) DefaultCapabilities(qwenEnableCompatible, qwenEnableCustomCompatible bool) map[string]string {
...
func (m *qwenProvider) OnRequestBody(ctx wrapper.HttpContext, apiName ApiName, body []byte) (types.Action, error) {
	if !m.config.isSupportedAPI(apiName) && !m.config.qwenEnableCustomCompatible {
...
func (m *qwenProvider) OnStreamingEvent(ctx wrapper.HttpContext, name ApiName, event StreamEvent) ([]StreamEvent, error) {
	if m.config.qwenEnableCompatible || m.config.qwenEnableCustomCompatible || name != ApiNameChatCompletion {
		return nil, nil
	}
