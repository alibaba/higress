...
func (b *baiduProviderInitializer) ValidateConfig(config ProviderConfig) error {
	if config.baiduRequestPath == "" {
		return errors.New("missing baiduRequestPath in provider config")
	}
	return nil
}
...
func (b *baiduProvider) OnRequestHeaders(ctx wrapper.HttpContext, apiName ApiName, log wrapper.Log) (types.Action, error) {
	...
	_ = util.OverwriteRequestPath(b.getRequestPath())
	// 使用文心一言接口协议 且 context为空时,跳过OnRequestBody()
	if b.config.protocol == protocolOriginal && b.config.context == nil {
...
func (b *baiduProvider) getRequestPath() string {
	return fmt.Sprintf("%s?access_token=%s", b.config.baiduRequestPath, b.config.GetRandomToken())
}
...
func (b *baiduProvider) responseBaidu2OpenAI(ctx wrapper.HttpContext, response *baiduTextGenResponse) *chatCompletionResponse {
	...
	return &chatCompletionResponse{
		...
		Model:             ctx.GetContext(ctxKeyOriginalRequestModel).(string),
		SystemFingerprint: "",
