...
func parseConfig(json gjson.Result, config *MyConfig, log wrapper.Log) error {
	...
	if chatgptUri == "" {
		config.ChatgptPath = "/api/v1/services/aigc/text-generation/generation"
		chatgptHost = "dashscope.aliyuncs.com"
	} else {
...
{"model":"%s","input":{"prompt":"%s"},"parameters":{}}`
...
func onHttpRequestHeaders(ctx wrapper.HttpContext, config MyConfig, log wrapper.Log) types.Action {
	...
	body := fmt.Sprintf(bodyTemplate, config.Model, prompt[0])
	//proxywasm.SendHttpResponse(200, nil, []byte(body), -1)
