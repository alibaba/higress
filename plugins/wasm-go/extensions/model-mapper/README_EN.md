...
| `enableResponseMapping` | bool         | Optional                           | true                            | Whether to enable response-side model rewrite. When enabled, if request model rewrite happened, upstream model names in responses are rewritten back to client model names (supports JSON and SSE). When disabled, only request rewrite is applied. |
