...
4. 只有当响应的 `Content-Type` 存在且与配置的 `response.body.contentTypes` 精确匹配时，才会记录响应体
5. 所有记录的内容都会被限制在配置的 `maxSize` 指定的大小内，且只保留流中的前 `maxSize` 字节
