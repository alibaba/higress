...
func (g *GithubProvider) ProcessExchangeToken(code string, cfg *Oatuh2Config, log *wrapper.Log) error {
	...
	cb := func(statusCode int, responseHeaders http.Header, responseBody []byte) {
		...
		if g.AccessToken == "" {
			SendError(log, string(responseBody), http.StatusUnauthorized)
			return
		}
		...
		cookieHeader, _ := buildSecureCookieHeader(g.AccessToken, cfg.Clientdomain, g.expires_in, cfg.SecureCookie)
