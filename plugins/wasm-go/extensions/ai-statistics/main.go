...
func onHttpStreamingBody(ctx wrapper.HttpContext, config AIStatisticsConfig, data []byte, endOfStream bool, log wrapper.Log) []byte {
	...
	if ctx.GetContext(StatisticsFirstTokenTime) == nil {
		...
		ctx.SetUserAttribute(LLMFirstTokenDuration, fmt.Sprint(firstTokenTime-requestStartTime))
	}

	// Set information about this request
	if model, inputToken, outputToken, ok := getUsage(data); ok {
		ctx.SetUserAttribute(Model, model)
		ctx.SetUserAttribute(InputToken, fmt.Sprint(inputToken))
		ctx.SetUserAttribute(OutputToken, fmt.Sprint(outputToken))
	}
	// If the end of the stream is reached, record metrics/logs/spans.
	if endOfStream {
		responseEndTime := time.Now().UnixMilli()
		ctx.SetUserAttribute(LLMServiceDuration, fmt.Sprint(responseEndTime-requestStartTime))
...
func writeMetric(ctx wrapper.HttpContext, config AIStatisticsConfig, log wrapper.Log) {
	...
	var model, inputToken, outputToken string
	if ctx.GetUserAttribute(Model) == nil || ctx.GetUserAttribute(InputToken) == nil || ctx.GetUserAttribute(OutputToken) == nil {
		log.Warnf("Get request usage information failed")
		return
	}
	model = ctx.GetUserAttribute(Model).(string)
	inputToken = ctx.GetUserAttribute(InputToken).(string)
	inputTokenUint64, err := strconv.ParseUint(inputToken, 10, 0)
	if err != nil || inputTokenUint64 == 0 {
		log.Warnf("inputToken convert failed, value is %d, err msg is [%v]", inputTokenUint64, err)
		return
	}
	outputToken = ctx.GetUserAttribute(OutputToken).(string)
	outputTokenUint64, err := strconv.ParseUint(outputToken, 10, 0)
	if err != nil || outputTokenUint64 == 0 {
		log.Warnf("outputToken convert failed, value is %d, err msg is [%v]", outputTokenUint64, err)
		return
	}
	...
	var llmFirstTokenDuration, llmServiceDuration string
	// Is stream response
	if ctx.GetUserAttribute(LLMFirstTokenDuration) != nil {
		llmFirstTokenDuration = ctx.GetUserAttribute(LLMFirstTokenDuration).(string)
		llmFirstTokenDurationUint64, err := strconv.ParseUint(llmFirstTokenDuration, 10, 0)
		if err != nil || llmFirstTokenDurationUint64 == 0 {
			log.Warnf("llmFirstTokenDuration convert failed, value is %d, err msg is [%v]", llmFirstTokenDurationUint64, err)
			return
		}
	...
	if ctx.GetUserAttribute(LLMServiceDuration) != nil {
		llmServiceDuration = ctx.GetUserAttribute(LLMServiceDuration).(string)
		llmServiceDurationUint64, err := strconv.ParseUint(llmServiceDuration, 10, 0)
		if err != nil || llmServiceDurationUint64 == 0 {
			log.Warnf("llmServiceDuration convert failed, value is %d, err msg is [%v]", llmServiceDurationUint64, err)
			return
		}
