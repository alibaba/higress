...
import (
	...
	"github.com/alibaba/higress/plugins/wasm-go/pkg/tokenusage"
...
func onHttpStreamingBody(ctx wrapper.HttpContext, config AIStatisticsConfig, data []byte, endOfStream bool) []byte {
	...
	if !config.disableOpenaiUsage {
		if usage := tokenusage.GetTokenUsage(data); usage.TotalToken > 0 {
			ctx.SetUserAttribute(Model, usage.Model)
			ctx.SetUserAttribute(InputToken, usage.InputToken)
			ctx.SetUserAttribute(OutputToken, usage.OutputToken)
			ctx.SetUserAttribute(TotalToken, usage.TotalToken)
			// Set span attributes for ARMS.
...
func onHttpResponseBody(ctx wrapper.HttpContext, config AIStatisticsConfig, body []byte) types.Action {
	...
	// Set information about this request
	if !config.disableOpenaiUsage {
		if usage := tokenusage.GetTokenUsage(body); usage.TotalToken > 0 {
			ctx.SetUserAttribute(Model, usage.Model)
			ctx.SetUserAttribute(InputToken, usage.InputToken)
			ctx.SetUserAttribute(OutputToken, usage.OutputToken)
			ctx.SetUserAttribute(TotalToken, usage.TotalToken)
...
func writeMetric(ctx wrapper.HttpContext, config AIStatisticsConfig) {
	...
	if ctx.GetUserAttribute(Model) == nil || ctx.GetUserAttribute(InputToken) == nil || ctx.GetUserAttribute(OutputToken) == nil || ctx.GetUserAttribute(TotalToken) == nil {
