...
func (w *watcher) getServiceCallback(server *McpServer, configGroup, dataId, path string) func(services []model.Instance, err error) {
	...
	return func(services []model.Instance, err error) {
		...
		if protocol == McpSSEProtocol {
			destinationRule := w.generateDrForSSEService(host)
			dr := &config.Config{
				Meta: config.Meta{
					GroupVersionKind: gvk.DestinationRule,
					Name:             fmt.Sprintf("%s-%s-%s", IstioMcpAutoGeneratedDrName, configGroup, strings.TrimSuffix(dataId, ".json")),
					Namespace:        w.namespace,
				},
...
func (w *watcher) buildVirtualServiceForMcpServer(serviceentry *v1alpha3.ServiceEntry, group, dataId, path, serverName string) *config.Config {
	...
	routeName := fmt.Sprintf("%s-%s-%s", IstioMcpAutoGeneratedHttpRouteName, group, strings.TrimSuffix(dataId, ".json"))
	baseUrl := w.McpServerBaseUrl
	if baseUrl == "" {
		baseUrl = "/mcp-registry"
	}
	mergePath := w.McpServerBaseUrl + "/" + serverName + "/" + strings.TrimPrefix(path, "/")
...
func (w *watcher) constructWasmFromMcpRule(cfg *WasmPluginConfig, group, dataId string) (*config.Config, error) {
