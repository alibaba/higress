...
var (
	...
	routeRewriteUpstreamTypes = map[string]bool{
		mcpserver.UpstreamTypeStreamable: true,
	}
	mcpServerRewriteUpstreamTypes = map[string]bool{
		mcpserver.UpstreamTypeSSE: true,
	}
)
...
func (w *watcher) buildVirtualServiceForMcpServer(serviceentry *v1alpha3.ServiceEntry, group, dataId, path string, server *provider.McpServer) *config.Config {
	...
	if routeRewriteUpstreamTypes[server.Protocol] {
...
func (w *watcher) buildMcpServerForMcpServer(vs *v1alpha3.VirtualService, group, dataId, path string, server *provider.McpServer) *config.Config {
	...
	httpRoute := vs.Http[0]
	pathMatchValue := httpRoute.Match[0].Uri.GetPrefix()
	pathRewritePrefix := ""
	if httpRoute.Rewrite != nil {
		pathRewritePrefix = httpRoute.Rewrite.Uri
	}
	...
	mcpServer := &mcpserver.McpServer{
		...
		UpstreamType:   protocolUpstreamTypeMapping[server.Protocol],
	}
	if mcpServerRewriteUpstreamTypes[mcpServer.UpstreamType] {
		mcpServer.EnablePathRewrite = len(pathRewritePrefix) != 0
