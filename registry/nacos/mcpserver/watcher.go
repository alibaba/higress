...
func (w *watcher) getServiceCallback(server *provider.McpServer, configGroup, dataId string) func(services []model.Instance) {
	...
	return func(services []model.Instance) {
		...
		mcpServer := w.buildMcpServerForMcpServer(vs.Spec.(*v1alpha3.VirtualService), configGroup, dataId, path, server.Name)
...
func (w *watcher) buildMcpServerForMcpServer(vs *v1alpha3.VirtualService, group, dataId, path, serverName string) *config.Config {
	if vs == nil {
		return nil
	}
	...
	if mcpServer.UpstreamType == mcpserver.UpstreamTypeSSE {
